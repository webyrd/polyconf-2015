...
;; restricted to single-argument closures
(define mapo
  (lambda (x body env^ ls val)
    (conde
      ((== '() ls) (== '() val))
      ((fresh (a d v res)
         (== `(,a . ,d) ls)
         (== `(,v . ,res) val)
         (eval-expo body `(ext-env ,x ,a ,env^) v)
         (mapo x body env^ d res))))))
...
(define eval-expo
  (lambda (exp env val)
    (conde
      ...
      ((fresh (f-e ls-e f ls x body env^)
         ;; map handles single-argument functions only
         (== `(map ,f-e ,ls-e) exp)
         (not-in-envo 'map env)
         (== `(closure (lambda (,x) ,body) ,env^) f)
         (symbolo x)
         (eval-expo f-e env f)
         (eval-expo ls-e env ls)
         (mapo x body env^ ls val)))
